...
import * as React from "react"
import {
  LayoutDashboard, ShoppingCart, Users, Package, AlertCircle, Shield,
  Settings, TrendingUp, Wallet, BarChart3, Store, Truck,
} from "lucide-react"
import { NavMain } from "@/components/nav-main"
import { NavUser } from "@/components/nav-user"
import {
...
export function AppSidebar({ ...props }) {
  ...
  // ── Wait for session before deciding role/routes (fixes manager→reseller race) ──
  if (isPending) {
    return null
  }
  ...
  const getBaseRoute = () => {
    switch (role) {
      ...
      default: return "/reseller"
    }
  }
  const baseRoute = getBaseRoute()

  // ── ADMIN — functional grouping ──
  const adminNav = [
    {
      title: "Overview",
      items: [
        { title: "Dashboard Overview", url: `${baseRoute}`, icon: LayoutDashboard, isBase: true },
      ],
    },
    {
      title: "Operations",
      items: [
        { title: "Orders", url: `${baseRoute}/orders`, icon: ShoppingCart },
        { title: "Subscriptions", url: `${baseRoute}/subscriptions`, icon: Package },
        { title: "Complaints", url: `${baseRoute}/complaints`, icon: AlertCircle },
      ],
    },
    {
      title: "People",
      items: [
        { title: "Resellers", url: `${baseRoute}/resellers`, icon: Users },
      ],
    },
    {
      title: "Finance",
      items: [
        { title: "Reseller Payouts", url: `${baseRoute}/confirm-payment/reseller-payout`, icon: Wallet },
        { title: "Supplier Payouts", url: `${baseRoute}/confirm-payment/supplier-payout`, icon: Truck },
      ],
    },
    {
      title: "Admin",
      items: [
        { title: "Users & Roles", url: `${baseRoute}/users`, icon: Shield },
        { title: "Settings", url: `${baseRoute}/settings`, icon: Settings },
      ],
    },
  ]

  // ── RESELLER — unchanged (original) ──
  const resellerNav = [
    {
      title: "Dashboard",
      items: [
  ...
  const managerNav = [
    ...
    },
    {
      title: "Finance",
      items: [
        { title: "Payout & Earnings", url: `${baseRoute}/payouts`, icon: Wallet },
      ],
    },
  ]
